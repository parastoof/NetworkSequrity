...
from networksequrity.entity.config_entity import DataValidationConfig
from networksequrity.entity.artifact_entity import DataIngestionArtifact, DataValidationArtifact
from networksequrity.constatnt.training_pipeline import SCHEMA_FILE_PATH
import os
import sys
from scipy.stats import ks_2samp
import pandas as pd
from networksequrity.utils.main_utils.utils import read_yaml_file, write_yaml_file
...
class DataValidation:
    ...
    def validate_number_of_column(self, dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Required nummber of columns: {number_of_column}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.validate_number_of_column(train_dataframe)
            if not status:
                error_message= "train dataframe does not contain all columns.\n"
            status=self.validate_number_of_column(test_dataframe)
            if not status:
